      ...
      <div className="flex flex-col items-center mt-10 w-full px-4">
        <form onSubmit={handleSearch} className="w-full max-w-md">
          <input
            ...
            value={searchText}
            onChange={(e) => { setSearchText(e.target.value); setIsEmptyStateActive(false); }}
        ...
        {bookDetails.length > 0 && (
         <div class="w-full max-w-4xl mt-6 max-h-96 overflow-y-auto bg-white rounded-lg shadow-md">
         <div class="flex flex-col">
           <div class="flex text-left text-xs font-medium text-gray-500 uppercase tracking-wider bg-gray-100 p-4">
             <div class="w-1/3 px-2">Title</div>
             <div class="w-1/3 px-2">Author Name</div>
             <div class="w-1/3 px-2">Publisher</div>
           </div>
           <div class="divide-y divide-gray-200">
             {bookDetails.map((ele, index) => (
               <div key={index} class="flex hover:bg-gray-50 text-sm text-gray-900 p-4">
                 <div class="w-1/3 px-2 whitespace-nowrap overflow-hidden overflow-ellipsis">
                   {ele?.title || 'NA'}
                 </div>
                 <div class="w-1/3 px-2 whitespace-nowrap overflow-hidden overflow-ellipsis">
